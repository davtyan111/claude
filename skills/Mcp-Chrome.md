...
It provides 20+ tools including screenshots, network monitoring, interactive operations, bookmark management, and semantic search capabilities powered by a built-in vector database with SIMD-accelerated AI operations. 
The server solves the problem of integrating AI agents with real-world browser automation while preserving user configurations, login states, and privacy through a fully local implementation.

For install 
claude mcp add --transport stdio hangwin-mcp-chrome uvx mcp-chrome

Chrome MCP Server 🚀
StarsLicense: MITTypeScriptChrome ExtensionRelease
...
Click the extension icon to open the plugin, then click connect to see the MCP configurationScreenshot 2025-06-09 15 52 06
